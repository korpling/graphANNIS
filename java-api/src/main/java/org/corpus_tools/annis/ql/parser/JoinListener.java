...
import org.corpus_tools.annis.ql.AqlParser.IndirectPartOfSubcorpusContext;
import org.corpus_tools.annis.ql.AqlParser.RangePartOfSubcorpusContext;
...
import org.corpus_tools.annis.ql.model.PartOfSubcorpus;
...
public class JoinListener extends AqlParserBaseListener {
    ...
    /**
     * An array which has an entry for each alternative. Each entry maps node
     * variables to a collection of query nodes.
     */
     ...
     * 
     * @param data
     *            The {@link QueryData} containing the already parsed nodes.
     * @param precedenceBound
     *            maximal range of precedence
     * @param tokenPositionToNode
     *            maps a token interval to a query nodes
     */
        ...
        for (List<QueryNode> alternative : data.getAlternatives()) {
            ...
            for (QueryNode n : alternative) {
                if (m.containsKey(n.getVariable())) {
                    throw new AnnisQLSemanticsException(n,
    ...
    public void enterBindingRelation(AqlParser.BindingRelationContext ctx) {
        ...
        for (int i = 0; i < numOfReferences; i++) {
            ...
            if (n == null) {
                throw new AnnisQLSemanticsException(AnnisParserAntlr.getLocation(ctx.getStart(), ctx.getStop()),
    ...
    public void enterRangePrecedence(AqlParser.RangePrecedenceContext ctx) {
        ...
        if (range.getMin() == 0 || range.getMax() == 0) {
            ...
            left.addOutgoingJoin(
                    addParsedLocation(ctx, new Precedence(right, range.getMin(), range.getMax(), segmentationName)));
    ...
    public void enterRangeNear(AqlParser.RangeNearContext ctx) {
        ...
        if (range.getMin() == 0 || range.getMax() == 0) {
            ...
            left.addOutgoingJoin(
                    addParsedLocation(ctx, new Near(right, range.getMin(), range.getMax(), segmentationName)));
    ...
    public void enterRangePointing(AqlParser.RangePointingContext ctx) {
        ...
        left.addOutgoingJoin(
                addParsedLocation(ctx, new PointingRelation(right, label, range.getMin(), range.getMax())));
    }
    ...
    public void enterDirectPartOfSubcorpus(DirectPartOfSubcorpusContext ctx) {
        ...
        Join j = new PartOfSubcorpus(right, 1);
    ...
    @Override
    public void enterIndirectPartOfSubcorpus(IndirectPartOfSubcorpusContext ctx) {
        QueryNode left = relationChain.get(relationIdx);
        QueryNode right = relationChain.get(relationIdx + 1);


        left.addOutgoingJoin(addParsedLocation(ctx, new PartOfSubcorpus(right)));
    }
    
    @Override
    public void enterRangePartOfSubcorpus(RangePartOfSubcorpusContext ctx) {
        QueryNode left = relationChain.get(relationIdx);
        QueryNode right = relationChain.get(relationIdx + 1);
        ...
        left.addOutgoingJoin(
                addParsedLocation(ctx, new PartOfSubcorpus(right, range.getMin(), range.getMax())));
    }
     ...
     *
     * This will automatically get the left and right hand refs and will construct a
     * new join specified by the type using reflection.
     * 
    ...
    private LinkedList<QueryAnnotation> fromRelationAnnotation(AqlParser.EdgeSpecContext ctx) {
        ...
        for (AqlParser.EdgeAnnoContext annoCtx : ctx.edgeAnno()) {
            ...
            QueryNode.TextMatching matching = QueryNodeListener.textMatchingFromSpec(annoCtx.value,
                    annoCtx.NEQ() != null);
    ...
    /*
     * private String errorRHS(String function) { return function +
     * " operator needs a right-hand-side"; }
     */
}
