...
public class SemanticValidator implements QueryDataTransformer {
  ...
  public void checkAlternative(QueryData data, List<QueryNode> alternative, int alternativeIndex,
    ...
    if (!invalidNames.isEmpty()) {
      throw new AnnisQLSemanticsException("The following variable names are " + "used for more than one node: "
    ...
    for (QueryNode source : alternative) {
      for (Join join : source.getOutgoingJoins()) {
        if (join instanceof Inclusion || join instanceof SameSpan || join instanceof Overlap
            || join instanceof RightOverlap || join instanceof LeftOverlap || join instanceof RightAlignment
            || join instanceof LeftAlignment) {
  ...
  private void createTransitiveHull(QueryNode n, final Multimap<Long, QueryNode> connected, Set<Long> transitiveHull) {
