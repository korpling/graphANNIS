...
fn add_automatic_cov_edge_for_node<F>(
    ...
    load_rank_result: &index::LoadRankResult,
    progress_callback: &F,
) -> Result<()>
where
    F: Fn(&str) -> (),
{
    ...
    for i in left_tok_pos.val..=right_tok_pos.val {
        ...
        if n != *tok_id {
            ...
            if !load_rank_result.is_text_coverage(&edge)? {
                let component_name = if load_rank_result.has_outgoing_text_coverage_edge(n)? {
...
fn load_edge_annotation<F>(
    path: &PathBuf,
    updater: &mut ChunkUpdater,
    rank_result: &index::LoadRankResult,
    id_to_node_name: &FxHashMap<NodeID, String>,
    is_annis_33: bool,
    progress_callback: &F,
) -> Result<()>
    ...
    F: Fn(&str) -> (),
{
    ...
    for result in edge_anno_tab_csv.records() {
        ...
        let pre: u32 = line.get(0).ok_or("Missing column")?.parse()?;
        if let Some(c) = rank_result.get_component_by_pre(pre)? {
            if let Some(e) = rank_result.get_edge_by_pre(pre)? {
