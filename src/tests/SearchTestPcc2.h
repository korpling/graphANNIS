...
TEST_F(SearchTestPcc2, StructureInclusionFilter) {
  ...
  q.addOperator(std::make_shared<Inclusion>(db, db.edges.getFunc), n1, n2, true);
...
TEST_F(SearchTestPcc2, IndirectPointingNested) {
  ...
  q.addOperator(std::make_shared<Pointing>("anaphor_antecedent", db.edges.getAllFunc, db.strings, 1, uintmax), 1, 0, true);
...
TEST_F(SearchTestPcc2, DirectPointingNested) {
  ...
  q.addOperator(std::make_shared<Pointing>("anaphor_antecedent", db.edges.getAllFunc, db.strings, 1, 1), 1, 0, true);
...
TEST_F(SearchTestPcc2, DirectPointingWithAnnoNested) {
  ...
  std::shared_ptr<Operator> op =
          ...
          "dep",
          db.edges.getAllFunc, db.strings,
