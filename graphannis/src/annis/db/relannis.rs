...
struct LoadRankResult {
    ...
    nodes_with_cov_or_dom_edges: DiskMap<NodeID, bool>,
    ...
    for i in left_tok_pos.val..=right_tok_pos.val {
        ...
        if n != tok_id {
            updates.add_event(UpdateEvent::AddEdge {
                ...
                component_name: "autogenerated-coverage".into(),
            })?;
        }
    }
    ...
    F: Fn(&str),
{
    ...
    for (n, node_name) in load_node_and_corpus_result.id_to_node_name.try_iter()? {
        // Do not calculate automatic coverage edges for token and nodes that
        // already have any outgoing coverage or dominance edges
        if !load_node_and_corpus_result
            .textpos_table
            .token_to_index
            .try_contains_key(&n)?
            && !load_rank_result
                .nodes_with_cov_or_dom_edges
                .try_contains_key(&n)?
        {
            if let Err(e) =
                add_automatic_cov_edge_for_node(updates, n, &node_name, load_node_and_corpus_result)
            {
    ...
    F: Fn(&str),
{
    ...
    let mut load_rank_result = LoadRankResult {
        ...
        nodes_with_cov_or_dom_edges: DiskMap::default(),
        ...
        if let Some(parent_as_str) = get_field(&line, pos_parent, "parent", &rank_tab_path)? {
            ...
            if let Some(source) = pre_to_node_id.get(&parent) {
                ...
                if let Some(c) = component_by_id.get(&component_ref) {
                    ...
                    if c.get_type() == AnnotationComponentType::Dominance
    ...
    load_rank_result.nodes_with_cov_or_dom_edges.compact()?;
    load_rank_result
        .component_for_parentless_target_node
        .compact()?;
