...
fn find_extra_all_nouns_gum(bench: &mut Criterion) {
    if CORPUS_STORAGE.is_none() {
        return;
    }
    ...
    bench.bench_function("find_extra_all_nouns_gum", move |b| {
        cs.preload("GUM").unwrap();
        b.iter(|| {
            let query = SearchQuery {
                corpus_names: &["GUM"],
            ...
            let f = cs.find_extra(query, 0, None, ResultOrder::Normal);
            assert!(f.is_ok());
        })
    });
}
...
fn find_extra_first_ten_nouns_gum(bench: &mut Criterion) {
    if CORPUS_STORAGE.is_none() {
        return;
    }
    ...
    bench.bench_function("find_extra_first_ten_nouns_gum", move |b| {
        cs.preload("GUM").unwrap();
        b.iter(|| {
            let query = SearchQuery {
                corpus_names: &["GUM"],
            ...
            let f = cs.find_extra(query, 0, Some(10), ResultOrder::Normal);
            assert!(f.is_ok());
        })
    });
}
...
fn find_extra_first_ten_token_gum(bench: &mut Criterion) {
    if CORPUS_STORAGE.is_none() {
        return;
    }
    ...
    bench.bench_function("find_extra_first_ten_token_gum", move |b| {
        cs.preload("GUM").unwrap();
        b.iter(|| {
            let query = SearchQuery {
                corpus_names: &["GUM"],
...
criterion_group!(name=default; config= Criterion::default().sample_size(50); targets =
    ...
    find_all_nouns_gum,
    find_extra_first_ten_nouns_gum,
    find_extra_first_ten_token_gum,
    find_extra_all_nouns_gum,
