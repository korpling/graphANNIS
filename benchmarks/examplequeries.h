...
public:
  ...
  {
    Query q(db);
    auto n1 = q.addNode(std::make_shared<AnnotationNameSearch>(db, "default_ns", "pos", "NN"));
    auto n2 = q.addNode(std::make_shared<AnnotationNameSearch>(db, "default_ns", "norm", "Blumen"));
  ...
  {
    Query q(db);
    q.addNode(std::make_shared<AnnotationNameSearch>(db, "cat"));
    return q;
  }
